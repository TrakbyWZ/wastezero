...
Add any other variables your team uses (see the root **`.env.example`**) as needed. Optional: SMTP and `OTP_FROM_EMAIL` for real one-time login emails. If SMTP is not set, **OTP codes are printed in the dev server** terminal. Optional: `LOG_FILES_INGEST_API_KEY` to exercise `/api/log-files/ingest` with `X-API-Key` (for example from the **.NET** Windows upload service against `http://localhost:3000` — in `windows-upload-service-dotnet/src/WasteZero.WindowsUploadService`, run **`Copy-Item appsettings.Development.json.example appsettings.Development.json`** (PowerShell) or copy the file manually so you have **`appsettings.Development.json`** (gitignored), edit paths and set **`UploadService:ApiKey`** to match this variable, or use optional `dotnet user-secrets`; see **`windows-upload-service-dotnet/README.md`**, section *Testing with local Next.js and Supabase*).
