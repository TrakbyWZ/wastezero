...
Add any other variables your team uses (see the root **`.env.example`**) as needed. Optional: SMTP and `OTP_FROM_EMAIL` for real one-time login emails — see [Email / SMTP Setup](./email-smtp-setup.md) to wire up a Google or Outlook account. If SMTP is not set, **OTP codes are printed in the dev server** terminal. Optional: `LOG_FILES_INGEST_API_KEY` to exercise `/api/log-files/ingest` with `X-API-Key` (for example from the **.NET** Windows upload service against `http://localhost:3000`). Configure the worker with **`appsettings.Development.json`** (copy from `appsettings.Development.json.example`, gitignored) and matching **`UploadService:ApiKey`** — full steps are in [Windows Upload Service](./windows-upload-service.md).
